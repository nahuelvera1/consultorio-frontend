...
function Ficha() {
  ...
  const cargarHistoria = () => {
    fetch(`http://localhost:3000/historia/${id}`).then(res => res.json()).then(setHistoria);
  }
  const cargarArchivos = () => {
    fetch(`http://localhost:3000/archivos/${id}`).then(res => res.json()).then(setArchivos);
  }
  ...
  const handleSubirArchivo = (e) => {
    ...
    fetch('http://localhost:3000/subir-archivo', { method: 'POST', body: formData })
            ...
            <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: '10px'}}>
                {archivos.map(a => (
                    <div key={a.id} style={{border: '1px solid #ddd', borderRadius: '5px', overflow: 'hidden', background:'white'}}>
                        <a href={`http://localhost:3000/uploads/${a.ruta_archivo}`} target="_blank" rel="noreferrer" style={{textDecoration:'none'}}>
                            {a.nombre_archivo.match(/\.(jpg|jpeg|png)$/i) ? <img src={`http://localhost:3000/uploads/${a.ruta_archivo}`} style={{width: '100%', height: '100px', objectFit: 'cover'}} /> : <div style={{height: '100px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f5f5f5', color:'#555'}}>📄</div>}
