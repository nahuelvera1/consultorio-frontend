...
function Panel() {
  ...
  const cargarMisTurnos = (id) => {
    fetch(`http://localhost:3000/turnos-paciente/${id}`).then(res => res.json()).then(setMisTurnos)
  }
  const cargarDentistas = () => {
    fetch('http://localhost:3000/usuarios').then(res => res.json()).then(data => setDentistas(data.filter(u => u.rol_id === 2)))
  }
  ...
  useEffect(() => {
    if (nuevoTurno.dentista_id && nuevoTurno.fecha) {
        fetch(`http://localhost:3000/horarios-disponibles?fecha=${nuevoTurno.fecha}&dentista_id=${nuevoTurno.dentista_id}`)
  ...
  const confirmarCancelacion = () => {
    ...
    fetch(`http://localhost:3000/cancelar-turno/${turnoParaCancelar.id}`, { method: 'PUT' }).then(() => { cargarMisTurnos(usuario.id); setModalVisible(false); setTurnoParaCancelar(null); })
  }
